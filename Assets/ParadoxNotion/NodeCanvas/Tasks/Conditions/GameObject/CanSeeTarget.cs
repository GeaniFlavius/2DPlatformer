...
namespace NodeCanvas.Tasks.Conditions
{
    ...
    public class CanSeeTarget : ConditionTask<Transform>
    {
        ...
        protected override bool OnCheck() {
            ...
            if ( Vector3.Angle(t.position - agent.position, agent.right) < viewAngle.value ) {
                return true;
            }
        ...
        public override void OnDrawGizmosSelected() {
            if ( agent != null ) {
                ...
                Gizmos.DrawLine(agent.position + offset, agent.position + offset + ( agent.right * maxDistance.value ));
                Gizmos.DrawWireSphere(agent.position + offset + ( agent.right * maxDistance.value ), 0.1f);
